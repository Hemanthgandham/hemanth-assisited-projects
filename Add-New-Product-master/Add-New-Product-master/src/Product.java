...
public class Product extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/pets_schema","root","hemanth4409*");
			
			Statement st = con.createStatement();
			st.executeUpdate("use pets_schema");
			
			int res = st.executeUpdate("insert into pets_schema.product(id,Name,color,price)values('"+id_int+"','"+name+"','"+color+"','"+price_cost+"')");
			ResultSet rs = st.executeQuery("select * from pets_schema.product");
